...
final class Plugin {
	...
	private function has_dependencies(): bool {
		...
		if ( ! function_exists( 'wp_register_ability' ) ) {
			add_action(
				...
				static function () {
					wp_admin_notice(
						__( 'Abilities API not available (wp_register_ability function not found)', 'wp-mcp-toolkit' ),
						array(
							'type'    => 'error',
							'dismiss' => false,
						),
	...
	public function __clone() {
		_doing_it_wrong(
			...
			sprintf(
				...
				esc_html__( 'The %s class should not be cloned.', 'wp-mcp-toolkit' ),
				esc_html( self::class ),
			),
			'0.1.0'
		);
	...
	public function __wakeup() {
		_doing_it_wrong(
			...
			sprintf(
				// translators: %s: Class name.
				esc_html__( 'De-serializing instances of %s is not allowed.', 'wp-mcp-toolkit' ),
				esc_html( self::class ),
			),
			'0.1.0'
		);
