	...
	public function create_server( string $server_id, string $server_route_namespace, string $server_route, string $server_name, string $server_description, string $server_version, array $mcp_transports, ?string $error_handler, ?string $observability_handler = null, array $tools = array(), array $resources = array(), array $prompts = array(), ?callable $transport_permission_callback = null ) {
		...
		if ( ! class_exists( $error_handler ) ) {
			return new \WP_Error(
				...
				sprintf(
					...
					esc_html__( 'Error handler class "%s" does not exist.', 'wp-mcp-toolkit' ),
		...
		if ( ! in_array( McpErrorHandlerInterface::class, class_implements( $error_handler ) ?: array(), true ) ) {
			return new \WP_Error(
				...
				sprintf(
				/* translators: %s: error handler class name */
					esc_html__( 'Error handler class "%s" must implement the McpErrorHandlerInterface.', 'wp-mcp-toolkit' ),
					esc_html( $error_handler )
				)
			);
		}
		...
		if ( ! class_exists( $observability_handler ) ) {
			return new \WP_Error(
				...
				/* translators: %s: observability handler class name */
					esc_html__( 'Observability handler class "%s" does not exist.', 'wp-mcp-toolkit' ),
		...
		if ( ! in_array( McpObservabilityHandlerInterface::class, class_implements( $observability_handler ) ?: array(), true ) ) {
			return new \WP_Error(
				...
				sprintf(
				/* translators: %s: observability handler class name */
					esc_html__( 'Observability handler class "%s" must implement the McpObservabilityHandlerInterface interface.', 'wp-mcp-toolkit' ),
					esc_html( $observability_handler )
				)
			);
		}
		...
		if ( ! doing_action( 'mcp_adapter_init' ) ) {
			_doing_it_wrong(
				__FUNCTION__,
				esc_html__( 'MCP Servers must be created during the "mcp_adapter_init" action. Hook into "mcp_adapter_init" to register your server.', 'wp-mcp-toolkit' ),
				'0.1.0'
			);
			return new \WP_Error(
				'invalid_timing',
				esc_html__( 'MCP Server creation must be done during mcp_adapter_init action.', 'wp-mcp-toolkit' )
			);
		}
		...
		if ( isset( $this->servers[ $server_id ] ) ) {
			_doing_it_wrong(
				...
				// translators: %s: server ID
					esc_html__( 'Server with ID "%s" already exists. Each server must have a unique ID.', 'wp-mcp-toolkit' ),
					esc_html( $server_id )
				),
				'0.1.0'
			);
			return new \WP_Error(
				'duplicate_server_id',
				// translators: %s: server ID.
				sprintf( esc_html__( 'Server with ID "%s" already exists.', 'wp-mcp-toolkit' ), esc_html( $server_id ) )
			);
		}
